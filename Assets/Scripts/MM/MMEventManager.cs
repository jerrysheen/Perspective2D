...
using UnityEngine.Audio;
	...
	public static class MMEventManager 
	{
		...
		/// 给监听者发送消息
		/// </summary>
		/// <param name="newEvent">消息结构 struct</param>
		public static void TriggerEventToListener<MMEvent>(MMEvent newEvent, MMEventListenerBase listener) where MMEvent : struct
        {
            List<MMEventListenerBase> list;
            if (!_subscribersList.TryGetValue(typeof(MMEvent), out list))
#if EVENTROUTER_REQUIRELISTENER
            ...
            for (int i = 0; i < list.Count; i++)
            {
				MMEventListenerBase mmlistener = list[i];
				if(mmlistener == listener)
                {
                    (list[i] as MMEventListener<MMEvent>).OnMMEvent(newEvent);
                }
            }
